function pfm_parse_manual_decisions_filter(Ic,Col,MidthickSurfs,Priors,XLS,OutFile,OutDir,WorkbenchBinary,MinSize)
...
% write out the labeled (pre-filtering) cifti file;
%O.data(O.data==find(strcmp(Priors.NetworkLabels,'Noise')))=0;
ft_write_cifti_mod([OutDir '/Tmp.dtseries.nii'],O);

% spatial filtering on the labeled network map (post-matching):
% per-network removal of small clusters + nearest-neighbor dilation.
% note: this operates on network labels, so adjacent same-network
% communities are merged before the size threshold is applied.
if MinSize > 0
    pfm_spatial_filtering([OutDir '/Tmp.dtseries.nii'],OutDir,'Tmp.dtseries.nii',MidthickSurfs,MinSize,WorkbenchBinary);
end
